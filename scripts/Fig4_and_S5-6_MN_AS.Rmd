...
SE_sig_coord_bed$event_type <- factor(SE_sig_coord_bed$event_type, levels=c("upstream_intron","short_intron1","short_intron2","long_intron","downstream_intron"))

# Merge splicing index with intron group coordinates
SI_hg38 %>% inner_join(SE_sig_coord_bed, by=c("chrom","start","end","strand")) %>% 
  filter(event_name.y %in% SE_diff_introns_only$event_name) %>%
  dplyr::select(-event_name.x, -event_name.y, -gene) %>% distinct() %>%
